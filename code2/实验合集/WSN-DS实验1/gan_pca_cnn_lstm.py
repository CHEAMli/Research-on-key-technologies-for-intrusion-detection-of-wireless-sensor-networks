...
def run_experiment(n_repeats=5, save_results=True):
    ...
    for repeat in range(n_repeats):
        ...
        # 2. 预处理 - GAN + PCA
        print("\n[2/5] 应用预处理: GAN生成 + PCA")
        pipeline = GANPCAPipeline(n_components=0.95, gan_epochs=50)
        ...
        # 5. 记录结果
        result = {
            'experiment': 'GAN_PCA_CNN_LSTM',
            'preprocessing': 'GAN+PCA',
            'model': 'CNN_LSTM',
            'repeat': repeat + 1,
            'seed': seed,
    ...
    if save_results:
        ...
        filepath = f'./results/gan_pca_cnn_lstm_{timestamp}.csv'
