"""
实验组合 8: KMS+PCA + CNN-LSTM
预处理: KMeans-SMOTE + PCA
模型: CNN-LSTM
"""
import os
...
from preprocessing import KMSPCAPipeline
...
def run_experiment(n_repeats=5, save_results=True):
    ...
    """
    print("="*80)
    print("实验: KMeans-SMOTE + PCA + CNN-LSTM")
    print("="*80)
    ...
    for repeat in range(n_repeats):
        ...
        # 2. 预处理 - KMeans-SMOTE + PCA
        print("\n[2/5] 应用预处理: KMeans-SMOTE + PCA")
        pipeline = KMSPCAPipeline(n_components=0.95)
        ...
        # 5. 记录结果
        result = {
            'experiment': 'KMS_PCA_CNN_LSTM',
            'preprocessing': 'KMS+PCA',
            'model': 'CNN_LSTM',
            'repeat': repeat + 1,
            'seed': seed,
    ...
    if save_results:
        ...
        filepath = f'./results/kms_pca_cnn_lstm_{timestamp}.csv'
