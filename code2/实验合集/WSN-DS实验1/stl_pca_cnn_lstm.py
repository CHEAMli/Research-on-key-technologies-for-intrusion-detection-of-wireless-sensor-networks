"""
实验组合 4: STL+PCA + CNN-LSTM
预处理: SMOTE TomekLink + PCA
模型: CNN-LSTM
"""
import os
...
from preprocessing import STLPCAPipeline
...
def run_experiment(n_repeats=5, save_results=True):
    ...
    """
    print("="*80)
    print("实验: SMOTE TomekLink + PCA + CNN-LSTM")
    print("="*80)
    ...
    for repeat in range(n_repeats):
        ...
        # 2. 预处理 - SMOTE TomekLink + PCA
        print("\n[2/5] 应用预处理: SMOTE TomekLink + PCA")
        pipeline = STLPCAPipeline(n_components=0.95)
        ...
        # 5. 记录结果
        result = {
            'experiment': 'STL_PCA_CNN_LSTM',
            'preprocessing': 'STL+PCA',
            'model': 'CNN_LSTM',
            'repeat': repeat + 1,
            'seed': seed,
    ...
    if save_results:
        ...
        filepath = f'./results/stl_pca_cnn_lstm_{timestamp}.csv'
