...
def main():
    ...
    print("\n===== 四分类评估（排除灰洞）=====")
    print(report)
    with open(os.path.join(RESULTS_DIR, "cnn_lstm_four_class_report.txt"), "w", encoding="utf-8") as f:
        f.write(report)
    ...
    pd.DataFrame([summary]).to_csv(os.path.join(RESULTS_DIR, "cnn_lstm_grayhole_routing_summary.csv"), index=False, encoding="utf-8-sig")

    print("\nArtifacts saved to:")
    print("-", os.path.join(RESULTS_DIR, "cnn_lstm_binary_metrics.csv"))
    print("-", os.path.join(RESULTS_DIR, "cnn_lstm_four_class_report.txt"))
    ...
    print("-", os.path.join(RESULTS_DIR, "cnn_lstm_grayhole_routing_summary.csv"))
