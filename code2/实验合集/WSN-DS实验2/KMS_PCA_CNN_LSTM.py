...
class CNNLSTMClassifier(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, num_layers: int, num_classes: int, cnn_channels: int = 64):
        super().__init__()
        # CNN for local feature extraction
        # Input: [B, T, F] -> permute to [B, F, T] for Conv1d
        ...
        # Input: [B, T, cnn_channels]
        self.lstm = nn.LSTM(cnn_channels, hidden_dim, num_layers=num_layers, batch_first=True, dropout=0.0 if num_layers == 1 else 0.1)
    ...
    def forward(self, x):  # x: [B, T, F]
        ...
        cnn_out = self.cnn(x_cnn)  # [B, cnn_channels, T]
        cnn_out = cnn_out.permute(0, 2, 1)  # [B, T, cnn_channels]
...
def prepare_data(seq_len: int = 8, n_clusters: int = 10, pca_components: int = 10) -> Tuple[Tuple[np.ndarray, np.ndarray], Tuple[np.ndarray, np.ndarray], List[str]]:
    ...
    # Step 2: KMeans clustering for feature augmentation
    print(f"Applying KMeans clustering (n_clusters={n_clusters})...")
    kmeans = KMeans(n_clusters=n_clusters, random_state=42, n_init=10)
    train_clusters = kmeans.fit_predict(train_df_scaled[feature_cols])
    test_clusters = kmeans.predict(test_df_scaled[feature_cols])
    
    # Add cluster labels as a new feature
    train_df_scaled['cluster'] = train_clusters
    ...
    train_pca = pca.fit_transform(train_df_scaled[feature_cols_with_cluster])
    test_pca = pca.transform(test_df_scaled[feature_cols_with_cluster])
    
    print(f"Explained variance ratio: {pca.explained_variance_ratio_.sum():.4f}")
    
    # Replace original features with PCA features
    pca_cols = [f'pca_{i}' for i in range(pca_components)]
    train_df_pca = train_df_scaled[[label_col, id_col, time_col]].copy()
    test_df_pca = test_df_scaled[[label_col, id_col, time_col]].copy()
    
    for i, col in enumerate(pca_cols):
        train_df_pca[col] = train_pca[:, i]
        test_df_pca[col] = test_pca[:, i]

    # Build sequences with PCA features
    X_train, y_train = build_sequences(train_df_pca, seq_len, pca_cols, label_col=label_col, group_col=id_col, time_col=time_col)
    X_test, y_test = build_sequences(test_df_pca, seq_len, pca_cols, label_col=label_col, group_col=id_col, time_col=time_col)
    
    print("\n序列构建后的测试集标签分布:")
    from collections import Counter
    print(pd.Series(Counter(y_test)).sort_index())

    return (X_train, y_train), (X_test, y_test), pca_cols
...
def train_model(
    ...
    model = CNNLSTMClassifier(input_dim, hidden_dim, num_layers, num_classes, cnn_channels=cnn_channels).to(device)
...
def main():
    ...
    print("Loading and preparing data with KMeans + PCA...")
    (X_train, y_train), (X_test, y_test), feature_cols = prepare_data(seq_len=seq_len, n_clusters=n_clusters, pca_components=pca_components)
    ...
    print("Training model (CNN-LSTM with KMeans+PCA)...")
    model = train_model(X_train, y_train, input_dim=input_dim, num_classes=5, hidden_dim=hidden_dim, epochs=epochs, cnn_channels=cnn_channels)
    ...
    pd.DataFrame([binary_metrics]).to_csv(os.path.join(RESULTS_DIR, "cnn_lstm_kms_pca_binary_metrics.csv"), index=False, encoding="utf-8-sig")
    ...
    print("\n===== 四分类评估（排除灰洞）=====")
    print(report)
    with open(os.path.join(RESULTS_DIR, "cnn_lstm_kms_pca_four_class_report.txt"), "w", encoding="utf-8") as f:
        f.write(report)
    ...
    probs_attacks_only = np.column_stack([probs_attacks[:, 1], probs_attacks[:, 3], probs_attacks[:, 4]])
    n_classes = 3
    
    rankings = []
    for i in range(len(y_true_remapped)):
        true_label_mapped = y_true_remapped[i] - 1
    ...
    pd.DataFrame([multiclass_metrics]).to_csv(os.path.join(RESULTS_DIR, "cnn_lstm_kms_pca_multiclass_metrics.csv"), index=False, encoding="utf-8-sig")
    
    # 3. Unknown（Grayhole）拒识性能评估
    # 只在攻击样本上评估（排除Normal）
    mask_attacks = y_test != 0
    y_test_attacks = y_test[mask_attacks]
    y_maxp_attacks = y_maxp[mask_attacks]
    
    y_true_unknown_attacks = (y_test_attacks == 2).astype(int)
    ...
    pd.DataFrame([unknown_metrics]).to_csv(os.path.join(RESULTS_DIR, "cnn_lstm_kms_pca_unknown_metrics.csv"), index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame([summary]).to_csv(os.path.join(RESULTS_DIR, "cnn_lstm_kms_pca_grayhole_routing_summary.csv"), index=False, encoding="utf-8-sig")

    print("\nArtifacts saved to:")
    print("-", os.path.join(RESULTS_DIR, "cnn_lstm_kms_pca_binary_metrics.csv"))
    print("-", os.path.join(RESULTS_DIR, "cnn_lstm_kms_pca_four_class_report.txt"))
    print("-", os.path.join(RESULTS_DIR, "cnn_lstm_kms_pca_multiclass_metrics.csv"))
    print("-", os.path.join(RESULTS_DIR, "cnn_lstm_kms_pca_unknown_metrics.csv"))
    print("-", os.path.join(RESULTS_DIR, "cnn_lstm_kms_pca_grayhole_routing_summary.csv"))
