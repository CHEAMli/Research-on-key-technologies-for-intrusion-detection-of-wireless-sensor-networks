...
class RNNClassifier(nn.Module):
    ...
    def forward(self, x):  # x: [B, F]
        x = x.unsqueeze(1)  # [B, 1, F] - sequence length 1
        out, _ = self.rnn(x)
        h_last = out[:, -1, :]
...
def main():
    ...
    print(f"Training model (RNN, rnn_type={rnn_type})...")
    ...
    report = classification_report(y_true_known, y_pred_known, labels=sorted(set(y_true_known)), 
                                   target_names=type_names_known, digits=4, output_dict=False)
    print("\n===== 多分类评估（排除injection）=====")
    print(report)
    with open(os.path.join(RESULTS_DIR, "rnn_known_class_report.txt"), "w", encoding="utf-8") as f:
        f.write(report)
    ...
    pd.DataFrame([summary]).to_csv(os.path.join(RESULTS_DIR, "rnn_injection_routing_summary.csv"), index=False, encoding="utf-8-sig")
    ...
    print("-", os.path.join(RESULTS_DIR, "rnn_known_class_report.txt"))
    print("-", os.path.join(RESULTS_DIR, "rnn_injection_routing_summary.csv"))
