...
class CNNLSTMClassifier(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, num_layers: int, num_classes: int, cnn_channels: int = 64):
        super().__init__()
        # CNN for feature extraction (treat features as a 1D sequence)
    ...
    def forward(self, x):  # x: [B, F]
        # Reshape for CNN: [B, 1, F]
        x = x.unsqueeze(1)  # [B, 1, F]
        x_cnn = self.cnn(x)  # [B, cnn_channels, F]
        x_cnn = x_cnn.permute(0, 2, 1)  # [B, F, cnn_channels]
        
        # LSTM
        lstm_out, _ = self.lstm(x_cnn)  # [B, F, hidden_dim]
...
def prepare_data() -> Tuple[Tuple[np.ndarray, np.ndarray, np.ndarray], Tuple[np.ndarray, np.ndarray, np.ndarray], LabelEncoder]:
    print("Loading data...")
    ...
    print(f"Train type distribution:\n{train_df['type'].value_counts().sort_index()}")
    print(f"\nTest type distribution:\n{test_df['type'].value_counts().sort_index()}")
...
def main():
    ...
    unknown_threshold = float(os.environ.get("UNKNOWN_THRESH", 0.72))
    ...
    print(f"Training model (CNN-LSTM)...")
    
    model = train_model(X_train, y_train, input_dim, num_classes, hidden_dim=hidden_dim, epochs=epochs, cnn_channels=cnn_channels)
    ...
    pd.DataFrame([binary_metrics]).to_csv(os.path.join(RESULTS_DIR, "cnn_lstm_binary_metrics.csv"), index=False, encoding="utf-8-sig")
    ...
    print("\n===== 多分类评估（排除injection）=====")
    print(report)
    with open(os.path.join(RESULTS_DIR, "cnn_lstm_known_class_report.txt"), "w", encoding="utf-8") as f:
        f.write(report)
    ...
    pd.DataFrame([multiclass_metrics]).to_csv(os.path.join(RESULTS_DIR, "cnn_lstm_multiclass_metrics.csv"), index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame([unknown_metrics]).to_csv(os.path.join(RESULTS_DIR, "cnn_lstm_unknown_metrics.csv"), index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame([summary]).to_csv(os.path.join(RESULTS_DIR, "cnn_lstm_injection_routing_summary.csv"), index=False, encoding="utf-8-sig")
    
    print("\nArtifacts saved to:")
    print("-", os.path.join(RESULTS_DIR, "cnn_lstm_binary_metrics.csv"))
    print("-", os.path.join(RESULTS_DIR, "cnn_lstm_multiclass_metrics.csv"))
    print("-", os.path.join(RESULTS_DIR, "cnn_lstm_unknown_metrics.csv"))
    print("-", os.path.join(RESULTS_DIR, "cnn_lstm_known_class_report.txt"))
    print("-", os.path.join(RESULTS_DIR, "cnn_lstm_injection_routing_summary.csv"))
