...
def main():
    ...
    print(f"Training model (CNN-LSTM with KMeans+PCA)...")
    ...
    print("\n===== 多分类评估（排除injection）=====")
    print(report)
    with open(os.path.join(RESULTS_DIR, "cnn_lstm_kms_pca_known_class_report.txt"), "w", encoding="utf-8") as f:
        f.write(report)
    ...
    pd.DataFrame([summary]).to_csv(os.path.join(RESULTS_DIR, "cnn_lstm_kms_pca_injection_routing_summary.csv"), index=False, encoding="utf-8-sig")
    ...
    print("-", os.path.join(RESULTS_DIR, "cnn_lstm_kms_pca_known_class_report.txt"))
    print("-", os.path.join(RESULTS_DIR, "cnn_lstm_kms_pca_injection_routing_summary.csv"))
