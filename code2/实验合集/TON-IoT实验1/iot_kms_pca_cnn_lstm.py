"""
实验: KMS+PCA预处理 + CNN-LSTM模型 (IoT数据集)
数据集: Train_Test_IoT_datasets
"""
import numpy as np
import pandas as pd
...
# 实验配置
EXPERIMENT_NAME = "IoT_KMS_PCA_CNN_LSTM"
NUM_RUNS = 5
EPOCHS = 1
BATCH_SIZE = 128
...
print(f"数据集: Train_Test_IoT_datasets")
print(f"{'='*60}\n")
...
for run_idx in range(NUM_RUNS):
    ...
    # 数据预处理
    print("\n[1/3] 数据预处理 (KMS + PCA)...")
    pipeline = KMSPCAPipeline()
    X_train_processed = pipeline.fit_transform(X_train, y_train)
    X_val_processed = pipeline.transform(X_val)
    X_test_processed = pipeline.transform(X_test)
    y_train_processed = pipeline.y_resampled  # 使用重采样后的标签
...
output_file = f"{RESULTS_DIR}/iot_kms_pca_cnn_lstm_{timestamp}.csv"
