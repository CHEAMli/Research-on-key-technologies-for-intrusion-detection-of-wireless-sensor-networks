# -*- coding: utf-8 -*-
"""TON-IoT (train_test_network.csv) — KMS + PCA 预处理 + 两阶段训练（一次运行输出：二分类 + 攻击多分类）

你要的实验一（TON-IoT）KMS+PCA 版本。
输出口径与 trainTON_two_stage.py 保持一致：
  Stage-1: Normal vs Attack（二分类）
  Stage-2: Attack 多分类（只在 ground-truth Attack 窗口上评估） + 每类窗口数统计

预处理差异：
  baseline 特征工程（continuous + low-card one-hot + high-card hashing）
  -> StandardScaler(仅continuous)
  -> PCA（fit 在 train rows；transform 到 train/val/test）
  -> Stage-2 训练集 attack windows 上做 KMeansSMOTE（KMS）补齐

运行：
  1) 修改 CFG.data_csv 为你的 train_test_network.csv 路径
  2) 确保同目录有 model_TONKMS.py（或把 import 改成你的模型文件名）
  3) python train_TONKMS_two_stage_kms_pca.py
"""

import os
import warnings

# 避免 sklearn/MKL 在 Windows 下的 MiniBatchKMeans 线程相关告警（以及潜在内存泄漏提示）
...
# 你给的 KMS 模型文件
from model_TONKMS import AnomalyDetectionModel  # type: ignore


def safe_torch_load_state_dict(path: str, map_location):
    """Load a state_dict safely across PyTorch versions (avoid torch.load FutureWarning)."""
    try:
...
class CFG:
    ...
    # Feature encoding（保持 baseline 一致）
    hash_dim: int = 8
    ...
    # PCA
    pca_n_components: float | int = 16  # 例如 16；也可写 0.95 表示保留 95% 方差

    # KMS (Stage-2 训练集 attack windows)
    ...
    # Stage-2 reporting
    stage2_report_every: int = 5  # 每5轮打印一次 per-class report；0=只在最终TEST打印
...
def preprocess_ton_features(df: pd.DataFrame, cfg: CFG) -> Tuple[pd.DataFrame, List[str], List[str]]:
    ...
    if cfg.drop_text_cols:
        # 这些文本列 hashing 维度太小会丢信息，默认先丢掉（你之前 baseline 就是这么做的）
...
def train_loop(
    ...
    for ep in range(1, epochs + 1):
        ...
        val = eval_fn(model, val_loader, ep)
...
# 5) 主流程
# =========================
def _fit_pca(X_tr: np.ndarray, pca_n_components: float | int) -> PCA:
    pca = PCA(n_components=pca_n_components, svd_solver="full", random_state=42)
    pca.fit(X_tr)
    return pca
...
def main():
    ...
    if "label" not in df.columns:
        raise ValueError("TON-IoT CSV 里未找到 label 列（需要 0/1）。")
    if "type" not in df.columns:
        raise ValueError("TON-IoT CSV 里未找到 type 列（攻击类型）。")
    ...
    # 2) split on binary label (stratified) + 保持子集内部按原始行序
    ...
    # 4) PCA（fit on train rows only）
    pca = _fit_pca(X_all[idx_tr], cfg.pca_n_components)
    X_all_pca = pca.transform(X_all).astype(np.float32)
    exp = float(np.sum(pca.explained_variance_ratio_))
    print(f"[PCA] {X_all.shape[1]} -> {X_all_pca.shape[1]} | explained_var={exp:.4f}")

    # 5) windows (two labels)
    Xw_tr, yb_tr, yt_tr = make_windows_two_labels(X_all_pca[idx_tr], y_bin[idx_tr], type_raw[idx_tr], cfg, attack_type_to_idx)
    Xw_val, yb_val, yt_val = make_windows_two_labels(X_all_pca[idx_val], y_bin[idx_val], type_raw[idx_val], cfg, attack_type_to_idx)
    Xw_te, yb_te, yt_te = make_windows_two_labels(X_all_pca[idx_test], y_bin[idx_test], type_raw[idx_test], cfg, attack_type_to_idx)
    ...
    best_bin = "best_ton_kms_stage1_binary.pt"
    train_loop(
        model_bin,
        dl_tr_bin,
        dl_val_bin,
        ...
        criterion=crit_bin,
        eval_fn=lambda m, l, ep: eval_binary(m, l, cfg.device),
        best_path=best_bin,
        best_key="f1",
    )
    ...
    def show_counts(title: str, y: np.ndarray):
        ...
        for i in range(K_attack):
            if cnt[i] > 0:
                print(f"  {attack_type_set[i]}: {int(cnt[i])}")
        print(f"  TOTAL: {int(cnt.sum())}")
        return cnt

    cnt_before = show_counts("Stage-2 train (before KMS)", yt_tr_a) if len(yt_tr_a) else None

    if len(yt_tr_a) == 0 or K_attack == 0:
        print("[WARN] No attack windows or no attack types found. Stage-2 skipped.")
        return

    # 2D for KMS: flatten windows
    X_flat = Xw_tr_a.reshape(Xw_tr_a.shape[0], -1)

    # k_neighbors 必须 < min_class_count
    min_cnt = int(np.min(cnt_before[cnt_before > 0])) if cnt_before is not None else 0
    ...
    if min_cnt <= 1 or k < 1:
        ...
        try:
            kms = KMeansSMOTE(random_state=cfg.kms_random_state, k_neighbors=k)
            ...
            print(f"[KMS] k_neighbors={k} | {Xw_tr_a.shape[0]} -> {Xw_tr_res.shape[0]}")
            show_counts("Stage-2 train (after KMS)", yt_tr_res)
        except Exception as e:
            print(f"[KMS][WARN] KMeansSMOTE failed ({type(e).__name__}: {e}). Use original train attack windows.")
            Xw_tr_res, yt_tr_res = Xw_tr_a, yt_tr_a
    ...
    best_type = "best_ton_kms_stage2_attack_types.pt"
    train_loop(
        model_type,
        dl_tr_type,
        dl_val_type,
        ...
        criterion=crit_type,
        eval_fn=lambda m, l, ep: eval_attack_multiclass(m, l, cfg.device, K_attack, attack_type_set, verbose=(cfg.stage2_report_every>0 and ep % cfg.stage2_report_every==0)),
        best_path=best_type,
        best_key="macro_f1",
    )

    te_type = eval_attack_multiclass(model_type, dl_te_type, cfg.device, K_attack, attack_type_set, verbose=True)
