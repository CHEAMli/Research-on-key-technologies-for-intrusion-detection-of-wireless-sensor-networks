...
from model_doubleSTL import AnomalyDetectionModel
...
def main():
    ...
    # ------------ 4.1 STL：对训练窗口做 SMOTETomek（只处理“无 padding”的完整窗口）------------
    W = config["window"]
    D = X_tr.shape[2]
    ...
    X_res, y_res = smt.fit_resample(X_full, y_full)

    X_res = X_res.reshape(-1, W, D)
    M_res = np.zeros((X_res.shape[0], W), dtype=bool)

    # 把“有 padding 的窗口”原样拼回去（避免 SMOTE 把 padding 区也合成出假数据）
    ...
    print("[STL] window counts after SMOTETomek:", dict(Counter(y_tr.tolist())))
