...
from model_doubleKMS import AnomalyDetectionModel
...
config = {
    ...
    # ===== Experiment-1 Preprocess Switch =====
    "use_kms_pca": True,
    "pca_n_components": 0.95,     # 或者改成整数：12/16/20

    # ===== KMS (KMeans-SMOTE) params =====
    "kms_k_neighbors": 3,         # 小类样本少时建议 2~3
    "kms_random_state": 42,

    # ===== Disable B4 augmentation for clean preprocess comparison =====
    "aug_shift_max": 0,
    "aug_prob": 0.0,
}
...
def main():
    ...
    # ------------ 2. KMS+PCA：填充 -> 标准化 -> PCA（仅在训练集fit）------------
    print("🔧 数据预处理（KMS+PCA）...")

    # 2.1 缺失值先填充（避免 scaler/pca 吃到 NaN）
    train_df = train_df.ffill().bfill()
    test_df = test_df.ffill().bfill()

    # 2.2 标准化（只在训练集 fit）
    scaler = StandardScaler()
    Xtr_scaled = scaler.fit_transform(train_df[feature_cols].to_numpy(dtype=np.float32))
    Xte_scaled = scaler.transform(test_df[feature_cols].to_numpy(dtype=np.float32))

    # 2.3 PCA（只在训练集 fit）
    pca = PCA(n_components=config.get("pca_n_components", 0.95), random_state=42)
    Xtr_pca = pca.fit_transform(Xtr_scaled)
    Xte_pca = pca.transform(Xte_scaled)

    pca_cols = [f"pca_{i}" for i in range(Xtr_pca.shape[1])]
    print(f"✅ PCA: {len(feature_cols)} -> {len(pca_cols)} | explained_var={pca.explained_variance_ratio_.sum():.4f}")

    # 用 PCA 特征替换原特征列
    train_df = train_df[[ID_COL, TIME_COL, LABEL_COL]].copy()
    test_df = test_df[[ID_COL, TIME_COL, LABEL_COL]].copy()
    train_df[pca_cols] = Xtr_pca
    test_df[pca_cols] = Xte_pca
    ...
    if config.get("use_kms_pca", False):
        ...
        if X_full.shape[0] > 0 and len(cnt_full) > 1:
            ...
            if k < 1:
                ...
                kms = KMeansSMOTE(random_state=int(config.get("kms_random_state", 42)), k_neighbors=k)
