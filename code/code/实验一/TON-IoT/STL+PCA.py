...
from imblearn.combine import SMOTETomek  # 导入SMOTE-TomekLink组合方法
...
# ================= 预处理函数 =================
def stl_pca_preprocess(X_train_raw, X_val_raw, X_test_raw, y_train, run):
    """使用SMOTE-TomekLink平衡数据 + PCA降维进行预处理"""
    # 1. 标准化
    ...
    # 2. 使用SMOTE-TomekLink处理训练集类别不平衡问题
    stl = SMOTETomek(random_state=42 + run)
    X_train_balanced, y_train_balanced = stl.fit_resample(X_train_scaled, y_train)

    # 打印平衡前后的样本分布变化
    print(f"\nSMOTE-TomekLink平衡前训练集样本数: {X_train_scaled.shape[0]}")
    print(f"SMOTE-TomekLink平衡后训练集样本数: {X_train_balanced.shape[0]}")
    print("平衡后类别分布:")
    for cls, count in zip(*np.unique(y_train_balanced, return_counts=True)):
    ...
    print(f"PCA降维前特征数: {X_train_balanced.shape[1]}")
    print(f"PCA降维后特征数: {X_train_pca.shape[1]} (保留95%方差)")

    return X_train_pca, X_val_pca, X_test_pca, y_train_balanced
...
# ================= 主实验函数 =================
def run_stl_pca_experiment():
    """运行STL+PCA预处理的实验"""
    print("=" * 80)
    print("TON-IoT 入侵检测实验 (STL+PCA预处理)")
    print("=" * 80)
    ...
    print(f"训练集: {len(X_train_raw)} 样本 ({len(X_train_raw) / total_samples * 100:.1f}%)")
    print(f"验证集: {len(X_val_raw)} 样本 ({len(X_val_raw) / total_samples * 100:.1f}%)")
    print(f"测试集: {len(X_test_raw)} 样本 ({len(X_test_raw) / total_samples * 100:.1f}%)")
    ...
    for run in range(5):
        ...
        # 预处理 (STL+PCA) - 修复：传入y_train并正确接收返回值
        try:
            X_train, X_val, X_test, y_train_balanced = stl_pca_preprocess(
                X_train_raw, X_val_raw, X_test_raw, y_train, run)
        except Exception as e:
            print(f"预处理失败 (轮次 {run + 1}): {e}")
            continue
    ...
    # ================= 计算平均结果 =================
    if not rf_results or not xgb_results:
        print("没有有效的实验结果可计算")
        return None
...
if __name__ == "__main__":
    try:
        # 运行STL+PCA预处理的实验
        results = run_stl_pca_experiment()
