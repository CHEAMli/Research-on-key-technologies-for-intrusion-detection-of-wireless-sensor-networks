...
def main_experiment():
    ...
    print(f"标准化后特征维度: {original_feature_count} (只进行标准化，不降维)")
    ...
    return {
        ...
        'feature_info': {
            ...
            'standardized': original_feature_count
        }
    }
...
if __name__ == "__main__":
    try:
        ...
        with open('experiment_results_standardized.json', 'w') as f:
        ...
        rf_class_df.to_csv('rf_class_metrics_standardized.csv', index=False)
        ...
        xgb_class_df.to_csv('xgb_class_metrics_standardized.csv', index=False)

        print("\n结果已保存到以下文件:")
        print("1. 'experiment_results_standardized.json' - 实验摘要")
        print("2. 'rf_class_metrics_standardized.csv' - RF每类别详细指标")
        print("3. 'xgb_class_metrics_standardized.csv' - XGB每类别详细指标")
